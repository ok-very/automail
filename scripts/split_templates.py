...
def write_docx_section(heading, paras, output_dir, seq_number):
    """Write a new DOCX file for a single H4 section with numbered filename"""
    doc = Document()
...
def process_docx(input_path, output_dir, headings_txt_path):
    """Split DOCX into sections based on H4 headings, number them, and log headings"""
    ...
    for para in doc.paragraphs:
        ...
        if style_name.startswith("Heading ") and style_name.endswith("4"):
...
if __name__ == "__main__":
    input_dir = "docs"          # folder with source DOCX files
    output_dir = "output_docx"  # folder to save split DOCX
    ...
    for fname in os.listdir(input_dir):
        if fname.lower().endswith(".docx"):
            ...
            process_docx(path, output_dir, headings_txt_path)
